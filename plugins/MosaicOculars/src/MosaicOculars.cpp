...
#include "MosaicOculars.hpp"
#include "MosaicOcularsGuiPanel.hpp"
...
StelModule* MosaicOcularsStelPluginInterface::getStelModule() const
{
	return new MosaicOculars();
}

StelPluginInfo MosaicOcularsStelPluginInterface::getPluginInfo() const
{
	// Allow to load the resources when used as a dynamic plugin
	Q_INIT_RESOURCE(MosaicOculars);

	StelPluginInfo info;
	info.id = "MosaicOculars";
	info.displayedName = N_("Mosaic Oculars");
	info.authors = "Timothy Reaves, MosaicOculars Developers";
	info.contact = "https://stellarium.org";
	info.description = N_("Shows the sky as if looking through a telescope eyepiece. (Only magnification and field of view are simulated.) It can also show a sensor frame, Telrad sight, and mosaic planning with configurable X/Y panels and overlap.");
	info.version = MOSAICOCULARS_PLUGIN_VERSION;
	info.license = MOSAICOCULARS_PLUGIN_LICENSE;
	info.startByDefault = false; // Don't auto-start, user must enable
	return info;
}

MosaicOculars::MosaicOculars()
	: selectedCCDIndex(-1)
...
QSettings* MosaicOculars::getSettings()
{
	return settings;
}
...
void MosaicOculars::deinit()
{
	...
	{
		qDebug() << "MosaicOculars::deinit() .. restoring skyDrawer values while ocular view is active";
...
void MosaicOculars::setFontSize(const int fontSize, const int guiPanelFontSize)
{
...
void MosaicOculars::setFontSizeFromApp(const int size)
{
...
void MosaicOculars::draw(StelCore* core)
{
	if (flagShowTelrad)
	{
		paintTelrad();
...
double MosaicOculars::getCallOrder(StelModuleActionName actionName) const
{
...
void MosaicOculars::update(double deltaTime)
{
	Q_UNUSED(deltaTime)
	...
	if (firstUpdate)
	{
		qWarning() << "MosaicOculars::update() CALLED FOR FIRST TIME!";
		qWarning() << "  flagGuiPanelEnabled =" << flagGuiPanelEnabled;
		qWarning() << "  guiPanel =" << (void*)guiPanel;
		firstUpdate = false;
	}
	updateCount++;

	// Try to create GUI panel if it was requested but not created yet
	// Dynamic plugins are initialized before StelGui is ready, so we keep checking until it's available
	if (flagGuiPanelEnabled && !guiPanel)
	{
		if (updateCount < 10 || updateCount % 60 == 0)  // Log first 10 and then every 60 frames
		{
			qWarning() << "MosaicOculars::update() attempt" << updateCount << "- checking for StelGui...";
		}
		...
		if (gui)
		{
			if (updateCount < 10)
			{
				qWarning() << "  StelGui found! Checking initComplete()...";
				qWarning() << "  gui->initComplete() =" << gui->initComplete();
			}

			// Check if GUI initialization is complete
			if (gui->initComplete())
			{
				QGraphicsWidget* skyGui = gui->getSkyGui();
				if (skyGui)
				{
					qWarning() << "MosaicOculars: Creating GUI panel (StelGui is initialized and ready)";
					enableGuiPanel(true);
					if (guiPanel)
					{
						qWarning() << "MosaicOculars: GUI panel created successfully!";
					}
					else
					{
						qWarning() << "MosaicOculars: GUI panel is NULL after enableGuiPanel!";
					}
				}
				else if (updateCount < 10)
				{
					qWarning() << "  StelGui initialized but SkyGui is NULL";
				}
			}
			else if (updateCount < 10)
			{
				qWarning() << "  StelGui exists but initComplete() returned false - waiting...";
			}
		}
		else if (updateCount < 10)
		{
			qWarning() << "  StelGui is NULL";
		}
	}
}

void MosaicOculars::handleMouseClicks(class QMouseEvent* event)
{
	StelCore *core = StelApp::getInstance().getCore();
...
void MosaicOculars::updateLatestSelectedSSO()
{
...
void MosaicOculars::init()
{
	...
	try {
		qDebug() << "MosaicOculars::init() starting initialization";
		validateAndLoadIniFile();
		qDebug() << "MosaicOculars::init() validateAndLoadIniFile completed";
		// assume all is well
		ready = true;
		...
		pxmapOnIcon = new QPixmap(":/mosaicocular/bt_ocular_on.png");
		pxmapOffIcon = new QPixmap(":/mosaicocular/bt_ocular_off.png");
		qDebug() << "MosaicOculars::init() - Pixmaps loaded";

		ocularDialog = new MosaicOcularsDialog(this, &ccds, &oculars, &telescopes, &lenses);
		qDebug() << "MosaicOculars::init() - Dialog created";
		initializeActivationActions();
		qDebug() << "MosaicOculars::init() - Activation actions initialized";
		determineMaxEyepieceAngle();
		qDebug() << "MosaicOculars::init() - Max eyepiece angle determined";
		...
		flagGuiPanelEnabled = settings->value("enable_control_panel", true).toBool();

		// Note: GUI panel creation is deferred because dynamic plugins are initialized before StelGui is ready.
		// The panel will be created in update() once StelGui becomes available.
	...
	{
		qWarning() << "Unable to locate mosaicocular.ini file or create a default one for Ocular plugin: " << e.what();
		ready = false;
	}

	qDebug() << "MosaicOculars::init() - Loading protractor textures";
	auto& texMan = StelApp::getInstance().getTextureManager();
	protractorTexture = texMan.createTexture(":/mosaicocular/Protractor.png");
	protractorFlipHTexture = texMan.createTexture(":/mosaicocular/ProtractorFlipH.png");
	protractorFlipVTexture = texMan.createTexture(":/mosaicocular/ProtractorFlipV.png");
	protractorFlipHVTexture = texMan.createTexture(":/mosaicocular/ProtractorFlipHV.png");
	qDebug() << "MosaicOculars::init() - Protractor textures loaded";
	// enforce check existence of reticle for the current eyepiece
	updateOcularReticle();
	qDebug() << "MosaicOculars::init() - Ocular reticle updated";
	...
	qDebug() << "MosaicOculars::init() - Signals connected";
	StelCore *core = StelApp::getInstance().getCore();
	StelSkyDrawer *skyDrawer = core->getSkyDrawer();
	...
	qDebug() << "MosaicOculars::init() - Initialization complete! Ready =" << ready;
}

void MosaicOculars::determineMaxEyepieceAngle()
{
	if (ready)
	{
...
void MosaicOculars::instrumentChanged()
{
...
void MosaicOculars::setFlagScaleImageCircle(bool state)
{
	if (state)
	{
		determineMaxEyepieceAngle();
...
void MosaicOculars::setScreenFOVForCCD()
{
			...
			if (gui)
			{
				...
				if (skyGui)
				{
					...
					if (flagShowOculars)
						...
						guiPanel->showCcdGui();
					// Otherwise just show the button bar (panel is visible by default)
				}
				else
				{
					qWarning() << "MosaicOculars::enableGuiPanel: SkyGui not available yet, deferring panel creation";
				}
			}
			else
			{
				qWarning() << "MosaicOculars::enableGuiPanel: StelGui not available yet, deferring panel creation";
			}
		}
	}
	else
...
void MosaicOculars::retranslateGui()
{
	if (guiPanel)
	{
		// TODO: Fix this hack!
		...
		if (gui)
		{
			guiPanel = new MosaicOcularsGuiPanel(this, gui->getSkyGui());
			if (flagShowOculars)
				guiPanel->showOcularGui();
			else if (flagShowCCD)
				guiPanel->showCcdGui();
...
void MosaicOculars::updateOcularReticle(void)
{
	reticleRotation = 0.0;
...
void MosaicOculars::updateLists()
{
	if (oculars.isEmpty())
	{
		selectedOcularIndex = -1;
...
void MosaicOculars::ccdRotationReset()
{
...
void MosaicOculars::prismPositionAngleReset()
{
	if (selectedCCDIndex<0)
		return;
	CCD *ccd = ccds[selectedCCDIndex];
...
void MosaicOculars::setSelectedCCDRotationAngle(double angle)
{
	if (selectedCCDIndex<0)
		return;
...
double MosaicOculars::getSelectedCCDRotationAngle() const
{
	if (selectedCCDIndex<0)
		return 0.0;
	CCD *ccd = ccds[selectedCCDIndex];
...
void MosaicOculars::setSelectedCCDPrismPositionAngle(double angle)
{
	if (selectedCCDIndex<0)
		return;
...
double MosaicOculars::getSelectedCCDPrismPositionAngle() const
{
	if (selectedCCDIndex<0)
		return 0.0;
	CCD *ccd = ccds[selectedCCDIndex];
...
void MosaicOculars::enableOcular(bool enableOcularMode)
{
	if (enableOcularMode)
	{
...
void MosaicOculars::decrementCCDIndex()
{
	selectedCCDIndex--;
...
void MosaicOculars::decrementOcularIndex()
{
	selectedOcularIndex--;
	if (selectedOcularIndex == -1)
	{
...
void MosaicOculars::decrementTelescopeIndex()
{
...
void MosaicOculars::decrementLensIndex()
{
	selectedLensIndex--;
	if (selectedLensIndex == lenses.count())
	{
...
void MosaicOculars::rotateReticleClockwise()
{
	// Step: 5 degrees
	reticleRotation -= 5.0;
}

void MosaicOculars::rotateReticleCounterclockwise()
{
	// Step: 5 degrees
	reticleRotation += 5.0;
}

void MosaicOculars::displayPopupMenu()
{
...
void MosaicOculars::incrementCCDIndex()
{
	selectedCCDIndex++;
	if (selectedCCDIndex == ccds.count())
	{
...
void MosaicOculars::incrementOcularIndex()
{
	selectedOcularIndex++;
	if (selectedOcularIndex == oculars.count())
	{
...
void MosaicOculars::incrementTelescopeIndex()
{
	selectedTelescopeIndex++;
	if (selectedTelescopeIndex == telescopes.count())
	{
...
void MosaicOculars::incrementLensIndex()
{
	selectedLensIndex++;
	if (selectedLensIndex == lenses.count())
	{
...
void MosaicOculars::disableLens()
{
	selectedLensIndex = -1;
	emit selectedLensChanged(selectedLensIndex);
}

void MosaicOculars::rotateCCD(int amount)
{
	CCD *ccd = ccds[selectedCCDIndex];
	if (!ccd) return;
...
void MosaicOculars::rotatePrism(int amount)
{
	CCD *ccd = ccds[selectedCCDIndex];
	if (!ccd) return;
...
void MosaicOculars::selectCCDAtIndex(int index)
{
...
void MosaicOculars::selectOcularAtIndex(int index)
{
	if (selectedTelescopeIndex == -1)
		selectTelescopeAtIndex(0);
...
void MosaicOculars::selectTelescopeAtIndex(int index)
{
...
void MosaicOculars::selectLensAtIndex(int index)
{
...
void MosaicOculars::toggleCCD(bool show)
{
...
void MosaicOculars::toggleCCD()
{
	toggleCCD(!flagShowCCD);
}

void MosaicOculars::toggleCrosshairs(bool show)
{
...
void MosaicOculars::toggleTelrad(bool show)
{
...
void MosaicOculars::toggleTelrad()
{
	toggleTelrad(!flagShowTelrad);
}

void MosaicOculars::initializeActivationActions()
{
	...
	// NOTE: GUI elements in MosaicOcularsGuiPanel
...
bool MosaicOculars::isBinocularDefined()
{
...
QRect MosaicOculars::drawSensorFrameAndOverlay(const StelProjectorP& projector, const Mat4f& derotate,
...
void MosaicOculars::drawCirclesOfConstantAngularRadii(StelPainter& sPainter, const Mat4f& derotate, const std::vector<float>& angularRadii)
{
	sPainter.enableClientStates(true);
...
void MosaicOculars::drawOAG(const StelProjectorP& projector, const Mat4f& derotate,
...
void MosaicOculars::paintCCDBounds()
{
...
QVector<MosaicOculars::MosaicPanel> MosaicOculars::calculateMosaicPanels()
{
...
void MosaicOculars::paintMosaicBounds()
{
...
void MosaicOculars::drawMosaicPanelFrame(const MosaicPanel& panel, const StelProjectorP& projector, const CCD& ccd, const Lens* lens)
{
	Telescope *telescope = telescopes[selectedTelescopeIndex];
	if (!telescope) return;
...
void MosaicOculars::paintCrosshairs()
{
	StelCore *core = StelApp::getInstance().getCore();
...
void MosaicOculars::paintTelrad()
{
	if (!flagShowOculars)
	{
		StelCore *core = StelApp::getInstance().getCore();
...
void MosaicOculars::paintOcularMask(const StelCore *core)
{
...
void MosaicOculars::paintText(const StelCore* core)
{
	const StelProjectorP prj = core->getProjection(StelCore::FrameAltAz);
	StelPainter painter(prj);
...
void MosaicOculars::validateAndLoadIniFile()
{
	// Ensure the module directory exists
	StelFileMgr::makeSureDirExistsAndIsWritable(StelFileMgr::getUserDir()+"/modules/MosaicOculars");
	StelFileMgr::Flags flags = static_cast<StelFileMgr::Flags>(StelFileMgr::Directory|StelFileMgr::Writable);
	QString ocularIniPath = StelFileMgr::findFile("modules/MosaicOculars/", flags) + "mosaicocular.ini";
	...
	{
		QFile src(":/mosaicocular/default_ocular.ini");
		if (!src.copy(ocularIniPath))
		{
			qWarning() << "MosaicOculars::validateIniFile cannot copy default_ocular.ini resource to [non-existing] "
				      + ocularIniPath;
		}
		else
		{
			qWarning() << "MosaicOculars::validateAndLoadIniFile() copied default_ocular.ini to " << QDir::toNativeSeparators(ocularIniPath);
	...
	{
		qInfo().noquote() << "MosaicOculars::validateAndLoadIniFile() mosaicocular.ini exists at:" << QDir::toNativeSeparators(ocularIniPath) << ". Checking version...";
		...
		qInfo().noquote() << "MosaicOculars::validateAndLoadIniFile() found existing ini file version" << ocularsVersion;

		if (ocularsVersion < MIN_OCULARS_INI_VERSION)
		{
			qWarning() << "MosaicOculars::validateAndLoadIniFile() existing ini file version" << ocularsVersion
			...
			{
				qWarning() << "MosaicOculars::validateAndLoadIniFile() cannot move mosaicocular.ini resource to mosaicocular.ini.old at path" + QDir::toNativeSeparators(ocularIniPath);
			}
			else
			{
				qWarning() << "MosaicOculars::validateAndLoadIniFile() mosaicocular.ini resource renamed to mosaicocular.ini.old at path" + QDir::toNativeSeparators(ocularIniPath);
				QFile src(":/mosaicocular/default_ocular.ini");
				if (!src.copy(ocularIniPath))
				{
					qWarning() << "MosaicOculars::validateIniFile cannot copy default_ocular.ini resource to [non-existing]" + QDir::toNativeSeparators(ocularIniPath);
				}
				else
				{
					qDebug() << "MosaicOculars::validateAndLoadIniFile() copied default_ocular.ini to" << QDir::toNativeSeparators(ocularIniPath);
...
void MosaicOculars::unzoomOcular()
{
...
void MosaicOculars::zoom(bool zoomedIn)
{
...
void MosaicOculars::toggleLines(bool visible)
{
	if (flagShowTelrad)
		return;
...
void MosaicOculars::hideUsageMessageIfDisplayed()
{
	if (usageMessageLabelID > -1)
	{
		LabelMgr *labelManager = GETSTELMODULE(LabelMgr);
...
Lens* MosaicOculars::selectedLens()
{
...
QMenu* MosaicOculars::addLensSubmenu(QMenu* parent)
{
	Q_ASSERT(parent);
...
QMenu* MosaicOculars::addTelescopeSubmenu(QMenu *parent)
{
	Q_ASSERT(parent);
...
void MosaicOculars::setFlagDMSDegrees(const bool b)
{
	flagDMSDegrees = b;
...
bool MosaicOculars::getFlagDMSDegrees() const
{
	return flagDMSDegrees;
}

void MosaicOculars::setFlagHorizontalCoordinates(const bool b)
{
...
bool MosaicOculars::getFlagHorizontalCoordinates() const
{
	return flagHorizontalCoordinates;
}

void MosaicOculars::setFlagRequireSelection(const bool b)
{
...
bool MosaicOculars::getFlagRequireSelection() const
{
	return flagRequireSelection;
}

void MosaicOculars::setFlagAutoLimitMagnitude(const bool b)
{
...
bool MosaicOculars::getFlagAutoLimitMagnitude() const
{
	return flagAutoLimitMagnitude;
}

void MosaicOculars::setMagLimitStarsOcularsManual(double mag)
{
...
double MosaicOculars::getMagLimitStarsOcularsManual() const
{
	return magLimitStarsOculars;
}

void MosaicOculars::setFlagInitFovUsage(const bool b)
{
	flagInitFOVUsage = b;
...
bool MosaicOculars::getFlagInitFovUsage() const
{
	return flagInitFOVUsage;
}

void MosaicOculars::setFlagInitDirectionUsage(const bool b)
{
...
bool MosaicOculars::getFlagInitDirectionUsage() const
{
	return flagInitDirectionUsage;
}

void MosaicOculars::setFlagAutosetMountForCCD(const bool b)
{
...
bool MosaicOculars::getFlagAutosetMountForCCD() const
{
	return  flagAutosetMountForCCD;
}

void MosaicOculars::setFlagScalingFOVForTelrad(const bool b)
{
...
bool MosaicOculars::getFlagScalingFOVForTelrad() const
{
	return  flagScalingFOVForTelrad;
}

void MosaicOculars::setTelradFOV(const Vec4f &fov)
{
	telradFOV = fov;
...
Vec4f MosaicOculars::getTelradFOV() const
{
	return  telradFOV;
}

void MosaicOculars::setFlagScalingFOVForCCD(const bool b)
{
...
bool MosaicOculars::getFlagScalingFOVForCCD() const
{
	return  flagScalingFOVForCCD;
}

void MosaicOculars::setFlagMaxExposureTimeForCCD(const bool b)
{
...
bool MosaicOculars::getFlagMaxExposureTimeForCCD() const
{
	return  flagMaxExposureTimeForCCD;
}

void MosaicOculars::setFlagUseSemiTransparency(const bool b)
{
...
bool MosaicOculars::getFlagUseSemiTransparency() const
{
	return flagSemiTransparency;
}

void MosaicOculars::setTransparencyMask(const int v)
{
	transparencyMask = v;
...
int MosaicOculars::getTransparencyMask() const
{
	return transparencyMask;
}

void MosaicOculars::setFlagShowResolutionCriteria(const bool b)
{
...
bool MosaicOculars::getFlagShowResolutionCriteria() const
{
	return flagShowResolutionCriteria;
}

void MosaicOculars::setCcdCropOverlayHSize(int size) {
...
void MosaicOculars::setCcdCropOverlayVSize(int size) {
...
void MosaicOculars::setFlagShowCcdCropOverlay(const bool b)
{
...
bool MosaicOculars::getFlagShowCcdCropOverlay(void) const
{
	return flagShowCcdCropOverlay;
}

void MosaicOculars::setFlagShowCcdCropOverlayPixelGrid(const bool b)
{
...
bool MosaicOculars::getFlagShowCcdCropOverlayPixelGrid(void) const
{
	return flagShowCcdCropOverlayPixelGrid;
}


void MosaicOculars::setFlagShowFocuserOverlay(const bool b)
{
...
bool MosaicOculars::getFlagShowFocuserOverlay(void) const
{
	return flagShowFocuserOverlay;
}

void MosaicOculars::setFlagUseSmallFocuserOverlay(const bool b)
{
...
bool MosaicOculars::getFlagUseSmallFocuserOverlay(void) const
{
	return flagUseSmallFocuserOverlay;
}

void MosaicOculars::setFlagUseMediumFocuserOverlay(const bool b)
{
...
bool MosaicOculars::getFlagUseMediumFocuserOverlay(void) const
{
	return flagUseMediumFocuserOverlay;
}

void MosaicOculars::setFlagUseLargeFocuserOverlay(const bool b)
{
...
bool MosaicOculars::getFlagUseLargeFocuserOverlay(void) const
{
	return flagUseLargeFocuserOverlay;
}

void MosaicOculars::setFlagMosaicMode(const bool b)
{
	if (b != flagMosaicMode)
	{
		flagMosaicMode = b;
...
bool MosaicOculars::getFlagMosaicMode(void) const
{
	return flagMosaicMode;
}

void MosaicOculars::setMosaicPanelsX(const int panels)
{
...
int MosaicOculars::getMosaicPanelsX(void) const
{
	return mosaicPanelsX;
}

void MosaicOculars::setMosaicPanelsY(const int panels)
{
	const int clampedPanels = qBound(1, panels, 20);
...
int MosaicOculars::getMosaicPanelsY(void) const
{
	return mosaicPanelsY;
}

void MosaicOculars::setMosaicRotationAngle(const double angle)
{
...
double MosaicOculars::getMosaicRotationAngle(void) const
{
	return mosaicRotationAngle;
}

void MosaicOculars::setMosaicOverlapPercent(const double percent)
{
...
double MosaicOculars::getMosaicOverlapPercent(void) const
{
	return mosaicOverlapPercent;
}

void MosaicOculars::setFlagShowContour(const bool b)
{
	flagShowContour = b;
...
bool MosaicOculars::getFlagShowContour(void) const
{
	return flagShowContour;
}

void MosaicOculars::setFlagShowCardinals(const bool b)
{
	flagShowCardinals = b;
...
bool MosaicOculars::getFlagShowCardinals(void) const
{
	return flagShowCardinals;
}

void MosaicOculars::setFlagAlignCrosshair(const bool b)
{
...
bool MosaicOculars::getFlagAlignCrosshair(void) const
{
	return flagAlignCrosshair;
}

void MosaicOculars::setArrowButtonScale(const int val)
{
...
int MosaicOculars::getArrowButtonScale() const
{
	return arrowButtonScale;
}

void MosaicOculars::setFlagHideGridsLines(const bool b)
{
...
bool MosaicOculars::getFlagHideGridsLines() const
{
	return flagHideGridsLines;
}

QString MosaicOculars::getDimensionsString(double fovX, double fovY) const
{
...
void MosaicOculars::setFlagShowOcularsButton(bool b)
{
	StelGui* gui = dynamic_cast<StelGui*>(StelApp::getInstance().getGui());
	if (gui)
	{
...
void MosaicOculars::setGuiPanelFontSize(int size)
{
...
void MosaicOculars::toggleCropOverlay()
{
	setFlagShowCcdCropOverlay(!getFlagShowCcdCropOverlay());
}

void MosaicOculars::togglePixelGrid()
{
	setFlagShowCcdCropOverlayPixelGrid(!getFlagShowCcdCropOverlayPixelGrid());
}

void MosaicOculars::toggleFocuserOverlay()
{
	setFlagShowFocuserOverlay(!getFlagShowFocuserOverlay());
}

double MosaicOculars::computeLimitMagnitude(Ocular *ocular, Telescope *telescope)
{
...
void MosaicOculars::handleAutoLimitToggle(bool on)
{
	if (!flagShowOculars)
		return;
...
void MosaicOculars::handleStarMagLimitToggle(bool on)
{
	if (!flagShowOculars)
		return;
