...
MosaicOcularsGuiPanel::MosaicOcularsGuiPanel(MosaicOculars* plugin,
				 QGraphicsWidget *parent,
				 Qt::WindowFlags wFlags):
	QGraphicsWidget(parent, wFlags),
	mosaicOcularsPlugin(plugin),
	parentWidget(parent),
	borderPath(Q_NULLPTR)
{
	...
	Q_ASSERT(mosaicOcularsPlugin->actionShowOcular);
	buttonOcular = new StelButton(buttonBar,
				      QPixmap(":/mosaicocular/bt_ocular_on.png"),
				      QPixmap(":/mosaicocular/bt_ocular_off.png"),
				      QPixmap(":/graphicGui/miscGlow32x32.png"),
				      mosaicOcularsPlugin->actionShowOcular,
				      true); //No background
	buttonOcular->setToolTip(mosaicOcularsPlugin->actionShowOcular->getText());

	Q_ASSERT(mosaicOcularsPlugin->actionShowCrosshairs);
	buttonCrosshairs = new StelButton(buttonBar,
					  QPixmap(":/mosaicocular/bt_crosshairs_on.png"),
					  QPixmap(":/mosaicocular/bt_crosshairs_off.png"),
					  QPixmap(":/graphicGui/miscGlow32x32.png"),
					  mosaicOcularsPlugin->actionShowCrosshairs,
					  true);
	buttonCrosshairs->setToolTip(mosaicOcularsPlugin->actionShowCrosshairs->getText());
	buttonCrosshairs->setVisible(false);

	Q_ASSERT(mosaicOcularsPlugin->actionShowSensor);
	buttonCcd = new StelButton(buttonBar,
				   QPixmap(":/mosaicocular/bt_sensor_on.png"),
				   QPixmap(":/mosaicocular/bt_sensor_off.png"),
				   QPixmap(":/graphicGui/miscGlow32x32.png"),
				   mosaicOcularsPlugin->actionShowSensor,
				   true);
	buttonCcd->setToolTip(mosaicOcularsPlugin->actionShowSensor->getText());

	Q_ASSERT(mosaicOcularsPlugin->actionShowTelrad);
	buttonTelrad = new StelButton(buttonBar,
				      QPixmap(":/mosaicocular/bt_telrad_on.png"),
				      QPixmap(":/mosaicocular/bt_telrad_off.png"),
				      QPixmap(":/graphicGui/miscGlow32x32.png"),
				      mosaicOcularsPlugin->actionShowTelrad,
				      true);
	buttonTelrad->setToolTip(mosaicOcularsPlugin->actionShowTelrad->getText());

	Q_ASSERT(mosaicOcularsPlugin->actionConfiguration);
	buttonConfiguration = new StelButton(buttonBar,
					     QPixmap(":/mosaicocular/bt_settings_on.png"),
					     QPixmap(":/mosaicocular/bt_settings_off.png"),
					     QPixmap(":/graphicGui/miscGlow32x32.png"),
					     mosaicOcularsPlugin->actionConfiguration,
					     true);
	buttonConfiguration->setToolTip(mosaicOcularsPlugin->actionConfiguration->getText());
	...
	connect(mosaicOcularsPlugin, SIGNAL(selectedOcularChanged(int)),    this, SLOT(updateOcularControls()));
	connect(mosaicOcularsPlugin, SIGNAL(selectedCCDChanged(int)),       this, SLOT(updateCcdControls()));
	connect(mosaicOcularsPlugin, SIGNAL(selectedTelescopeChanged(int)), this, SLOT(updateTelescopeControls()));
	connect(mosaicOcularsPlugin, SIGNAL(selectedLensChanged(int)),      this, SLOT(updateTelescopeControls()));
	connect(mosaicOcularsPlugin, SIGNAL(selectedCCDRotationAngleChanged(double)),      this, SLOT(updateCcdControls()));
	connect(mosaicOcularsPlugin, SIGNAL(selectedCCDPrismPositionAngleChanged(double)), this, SLOT(updateCcdControls()));
	
	//Connect mosaic signals
	connect(mosaicOcularsPlugin, SIGNAL(flagMosaicModeChanged(bool)), this, SLOT(updateMosaicControls()));
	connect(mosaicOcularsPlugin, SIGNAL(mosaicPanelsXChanged(int)), this, SLOT(updateMosaicControls()));
	connect(mosaicOcularsPlugin, SIGNAL(mosaicPanelsYChanged(int)), this, SLOT(updateMosaicControls()));
...
MosaicOcularsGuiPanel::~MosaicOcularsGuiPanel()
{
	if (borderPath)
		delete borderPath;
...
void MosaicOcularsGuiPanel::showOcularGui()
{
	setPreferredHeight(0);//WTF?
	if (mosaicOcularsPlugin->flagShowOculars)
	{
...
void MosaicOcularsGuiPanel::showCcdGui()
{
	updateCcdControls();
}

void MosaicOcularsGuiPanel::foldGui()
{
	//qDebug() << "hidePanel()";
	setOcularControlsVisible(false);
...
void MosaicOcularsGuiPanel::updatePosition()
{
	updateGeometry();
	/*qDebug() << "Widget:" << size()
...
void MosaicOcularsGuiPanel::updateOcularControls()
{
	setCcdControlsVisible(false);

	//Get the name
	int index = mosaicOcularsPlugin->selectedOcularIndex;
	Ocular* ocular = mosaicOcularsPlugin->oculars[index];
	Q_ASSERT(ocular);
	...
	setOcularControlsVisible(mosaicOcularsPlugin->getEnableOcular());

	updateTelescopeControls();//Contains a call to updatePosition()
}

void MosaicOcularsGuiPanel::updateLensControls()
{
	Lens* lens = mosaicOcularsPlugin->selectedLens();
	int index = mosaicOcularsPlugin->selectedLensIndex;
	...
	int oindex = mosaicOcularsPlugin->selectedOcularIndex;
	Ocular* ocular = mosaicOcularsPlugin->oculars[oindex];
	if (mosaicOcularsPlugin->getEnableCCD() || mosaicOcularsPlugin->getEnableOcular())
	{
		if (ocular->isBinoculars() && mosaicOcularsPlugin->getEnableOcular()) // Hide the lens info for binoculars in eyepiece mode only
			setLensControlsVisible(false);
		else
...
void MosaicOcularsGuiPanel::updateCcdControls()
{
	setOcularControlsVisible(false);

	//Get the name
	int index = mosaicOcularsPlugin->selectedCCDIndex;
	CCD* ccd = mosaicOcularsPlugin->ccds[index];	
	Q_ASSERT(ccd);
	if (ccd->chipRotAngle()!=mosaicOcularsPlugin->getSelectedCCDRotationAngle())
		mosaicOcularsPlugin->setSelectedCCDRotationAngle(ccd->chipRotAngle());
	if (ccd->prismPosAngle()!=mosaicOcularsPlugin->getSelectedCCDPrismPositionAngle())
		mosaicOcularsPlugin->setSelectedCCDPrismPositionAngle(ccd->prismPosAngle());
	...
	index = mosaicOcularsPlugin->selectedTelescopeIndex;
	Telescope* telescope = mosaicOcularsPlugin->telescopes[index];
	Q_ASSERT(telescope);
	const double fovX = ccd->getActualFOVx(telescope, lens);
	const double fovY = ccd->getActualFOVy(telescope, lens);
	QString dimensionsLabel = QString(q_("Dimensions: %1")).arg(mosaicOcularsPlugin->getDimensionsString(fovX, fovY));
	...
	QString rotation = QString::number(mosaicOcularsPlugin->getSelectedCCDRotationAngle(), 'f', 0);
	...
	if (ccd->hasOAG())
	{
		...
		QString positionAngle = QString::number(mosaicOcularsPlugin->getSelectedCCDPrismPositionAngle(), 'f', 0);
	...
	setCcdControlsVisible(mosaicOcularsPlugin->getEnableCCD());
...
void MosaicOcularsGuiPanel::updateTelescopeControls()
{
	//Get the name
	int index = mosaicOcularsPlugin->selectedTelescopeIndex;
	Telescope* telescope = mosaicOcularsPlugin->telescopes[index];
	Q_ASSERT(telescope);
	...
	if (mosaicOcularsPlugin->flagShowCCD)
	{
		int indexCCD = mosaicOcularsPlugin->selectedCCDIndex;
		CCD* ccd = mosaicOcularsPlugin->ccds[indexCCD];
		Q_ASSERT(ccd);
	...
	if (mosaicOcularsPlugin->flagShowOculars)
	{
		...
		int indexOcular = mosaicOcularsPlugin->selectedOcularIndex;
		Ocular* ocular = mosaicOcularsPlugin->oculars[indexOcular];
	...
	if (diameter>0.0 && mosaicOcularsPlugin->getFlagShowResolutionCriteria() && !isBinocular)
	{
	...
	// Visual resolution
	if (mosaicOcularsPlugin->flagShowOculars && mosaicOcularsPlugin->getFlagShowResolutionCriteria() && diameter>0.0 && !isBinocular)
	{
	...
	if (mosaicOcularsPlugin->flagShowOculars && mosaicOcularsPlugin->getFlagAutoLimitMagnitude())
	{
		Ocular* ocular = mosaicOcularsPlugin->oculars[mosaicOcularsPlugin->selectedOcularIndex];
		QString limitMagnitudeLabel = QString("%1: %2").arg(qc_("Limiting magnitude", "Limiting magnitude of device"), QString::number(mosaicOcularsPlugin->getLimitMagnitude(ocular, telescope), 'f', 2));
...
void MosaicOcularsGuiPanel::setLensControlsVisible(bool show)
{
	if (show)
	{
...
void MosaicOcularsGuiPanel::setOcularControlsVisible(bool show)
{
	if (show)
	{
...
void MosaicOcularsGuiPanel::setCcdControlsVisible(bool show)
{
	if (show)
	{
...
void MosaicOcularsGuiPanel::setTelescopeControlsVisible(bool show)
{
	if (show)
	{
...
void MosaicOcularsGuiPanel::updateMosaicControls()
{
	if (!mosaicOcularsPlugin->getEnableCCD() || !ccdControls->isVisible())
	{
	...
	// Mosaic mode toggle
	QString mosaicModeLabel = mosaicOcularsPlugin->getFlagMosaicMode() ? q_("Mosaic: ON") : q_("Mosaic: OFF");
	...
	// X Panels control
	QString panelsXLabel = QString(q_("X Panels: %1")).arg(mosaicOcularsPlugin->getMosaicPanelsX());
	...
	// Y Panels control
	QString panelsYLabel = QString(q_("Y Panels: %1")).arg(mosaicOcularsPlugin->getMosaicPanelsY());
...
void MosaicOcularsGuiPanel::updateMainButtonsPositions()
{
...
void MosaicOcularsGuiPanel::setControlsColor(const QColor& color)
{
...
void MosaicOcularsGuiPanel::setControlsFont(const QFont& font)
{
	Q_ASSERT(fieldOcularName);
	Q_ASSERT(fieldOcularFl);
	Q_ASSERT(fieldOcularAfov);
...
void MosaicOcularsGuiPanel::setColorScheme(const QString &schemeName)
{
	Q_UNUSED(schemeName)
...
void MosaicOcularsGuiPanel::updateRotationButtons()
{
...
void MosaicOcularsGuiPanel::updateLayout()
{
...
void MosaicOcularsGuiPanel::setFontSize(const int size)
{
	QFont newFont = font();
...
QPixmap MosaicOcularsGuiPanel::createPixmapFromText(const QString& text,
					      int width,
					      int height,
					      const QFont& font,
